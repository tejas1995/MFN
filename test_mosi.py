...
class MFNPhantom(nn.Module):
	...
	def calc_phantom_loss(self, batchsize, X_test, y_test, mode='before'):
		...
		for batch in xrange(num_batches):
			...
			total_loss_a += self.phantom_a_criterion(pred_x_a, x_a).item()
			total_loss_v += self.phantom_v_criterion(pred_x_v, x_v).item()
	...
	def train_epoch(self, batchsize, X_train, y_train, optimizer):
		...
		for batch in xrange(num_batches):
			...
			predictions, pred_x_a, pred_x_v = self.forward(x_l, x_a, x_v, False, True)
	...
	def evaluate(self, X_valid, y_valid):
		...
		with torch.no_grad():
			...
			predictions, pred_x_a, pred_x_v = self.forward(x_l, x_a, x_v, True, True)
			predictions = predictions.squeeze(1)
	...
	def predict(self, X_test):
		...
		with torch.no_grad():
			...
			predictions, _, _ = self.forward(x_l, x_a, x_v, True, True)
			predictions = predictions.squeeze(1)
...
def train_mfn_phantom(X_train, y_train, X_valid, y_valid, X_test, y_test, configs):
	...
	#model = EFLSTM(d,h,output_dim,dropout)
	model = MFNPhantom(config,NN1Config,NN2Config,gamma1Config,gamma2Config,outConfig)
	...
	for epoch in range(config["num_epochs"]):
		...
		if valid_loss <= best_valid:
			...
			best_valid = valid_loss
			print epoch, train_loss, valid_loss, 'saving model'
			torch.save(model, 'temp_models/mfn_phantom_%d.pt' %rand)
			#best_model = copy.deepcopy(model).cpu().gpu()
		else:
			print epoch, train_loss, valid_loss

	print 'model number is:', rand
	model = torch.load('temp_models/mfn_phantom_%d.pt' %rand)
...
while True:
	...
	# acc 77.0 
	# configs = [{'input_dims': [300, 5, 20], 'batchsize': 128, 'memsize': 400, 
	# 'windowsize': 2, 'lr': 0.00001, 'num_epochs': 1000, 'h_dims': [64, 8, 80], 'momentum': 0.5}, 
	# {'shapes': 128, 'drop': 0.5}, {'shapes': 128, 'drop': 0.2}, 
	# {'shapes': 128, 'drop': 0.5}, {'shapes': 128, 'drop': 0.5}, 
	# {'shapes': 256, 'drop': 0.5}]

	# best hparam configs
	# configs = [{'input_dims': [300, 5, 20], 'batchsize': 64, 'memsize': 64,
	# 'windowsize': 2, 'lr': 1e-05, 'num_epochs': 1000, 'h_dims': [64, 32, 32], 'momentum': 0.9},
	# {'shapes': 64, 'drop': 0.5}, {'shapes': 64, 'drop': 0.5}, 
	# {'shapes': 32, 'drop': 0.5}, {'shapes': 256, 'drop': 0.5}, 
	# {'shapes': 128, 'drop': 0.5}]

	# max parameters
	# configs = [{'input_dims': [300, 5, 20], 'batchsize': 128, 'memsize': 400, 
	# 'windowsize': 2, 'lr': 0.005, 'num_epochs': 1000, 'h_dims': [256, 80, 80], 'momentum': 0.9}, 
	# {'shapes': 256, 'drop': 0.5}, {'shapes': 256, 'drop': 0.2}, 
